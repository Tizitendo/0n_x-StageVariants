-- log.info("Successfully loaded " .. _ENV["!guid"] .. ".") --not needed here because it's in the same mod, but needed for you
...
if hotload then
    --clear the content of all stagevariants, so there won't be duplicates when init gets run again
    for _, variant in ipairs(StageVariants.find_all()) do
